					...
					switch (str[offset])
					{
						...
						default:
							try
							{
								...
								{
									System.Int64.TryParse(str, out custom_l);
									System.Int32.TryParse(str, out custom_n);

									//if (str.Length >= 10)
									//    custom_l = System.Convert.ToInt64(str);
									//else custom_n = System.Convert.ToInt32(str);
								}
#if USEFLOAT
	...
	public void GetField(ref int field, string name, FieldNotFound fail)
	{
		if (type == Type.OBJECT)
		{
			...
			if (index >= 0)
			{
				...
				field = list[index].custom_n;
				return;
			}
		}
		if (fail != null) fail.Invoke(name);
	}
	public void GetField(ref long field, string name) { GetField(ref field, name, null); }
	public void GetField(ref long field, string name, FieldNotFound fail)
	{
		if (type == Type.OBJECT)
		{
			int index = keys.IndexOf(name);
	...
	{ //Convert the JSONObject into a string
		...
		switch (type)
		{
			...
			case Type.NUMBER:
				...
				{
					if (custom_l != 0) builder.Append(custom_l.ToString());
