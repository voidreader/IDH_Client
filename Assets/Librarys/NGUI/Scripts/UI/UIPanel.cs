...
public class UIPanel : UIRect
{
	...
	public override Vector3[] worldCorners
	{
		get
		{
			...
			{
                #region 기존 2019.03 버전에서 존재하던 패널 코드
                ...
                #endregion
                #region  2017ver에서 얻어온 PanelCode
                //[!]

                UIRoot rt = root;

                if (rt != null && rt.fitHeight && rt.fitWidth)
                {
                    float menualRatio = (float)rt.manualWidth / rt.manualHeight;
                    float screenRatio = (float)Screen.width / Screen.height;
                    float height = (menualRatio > screenRatio ? 2f / (menualRatio / screenRatio) : 2f);
                    float width = rt.manualWidth / (rt.manualHeight / height);
...
#region 2017ver에서 얻어온 PanelCode
        ...
        else if (anchorCamera != null && root.fitHeight && root.fitWidth)
		{
			UIRoot rt = root;
			...
			float h = (menualRatio > screenRatio ? 2f / (menualRatio / screenRatio) : 2f) * 0.5f;
			float w = rt.manualWidth / (rt.manualHeight / h);
